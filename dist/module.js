        ...
        execute: function() {
            ...
            DiscretePanelCtrl = (function (_super) {
                ...
                function DiscretePanelCtrl($scope, $injector, templateSrv, $sce) {
  				...
  				DiscretePanelCtrl.prototype.onInitPanelActions = function (actions) {
                    actions.push({ text: 'Export CSV', click: 'ctrl.exportCsv()' });
                };
				
				DiscretePanelCtrl.prototype.exportCsv = function () {
					console.log(this.data);
					var rows = [];
					if(this.data.length > 0){
						if(this.data[0].changes){
							if(this.data[0].changes.length > 0){
                                let tmpFlag = '';
								for(let i = 0 ; i < this.data[0].changes.length ; i++){
									let tmpData = this.data[0].changes[i];
                                    if(tmpData.val !== tmpFlag || i === this.data[0].changes.length-1){
                                        tmpFlag = tmpData.val;
                                        let valueArr = tmpData.val.split('(');
                                        let Status = valueArr[0];
                                        let Speed = '';
                                        try{
                                            Speed = valueArr[1].replace(')', '');
                                        }catch(error){
                                            Speed = Status;
                                        }
                                        let date = new Date(tmpData.start);
                                        let dateStr = date.getFullYear()+
                                          ...
                                          ":"+date.getMinutes()+
                                          ":"+date.getSeconds();                                        
                                        rows.push([dateStr, Speed, Status]);
                                    }
									//rows.push([dateStr, Speed, Status]);
									//console.log(tmpData);
								}
					...
					rows.unshift(["Time", "Status", "Speed"]);
					console.log(rows);
					...
					link.click();	
					//var test_data = [{"datapoints":[[1000,1621907107946]]}]
					//const scope = this.$scope.$new(true);
					//scope.seriesList = this.dataList;
					//console.log("DATA= " + JSON.stringify(link));
					//console.log(scope);
